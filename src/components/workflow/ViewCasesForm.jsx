...
import { PageCard, SelectField, Snackbar } from '@/components/ui/PageComponents';
...
export default function ViewCasesForm() {
  ...
  useEffect(() => {
    ...
    getDocs(collection(db, 'Types')).then((snap) => {
      setTypes(snap.docs.map((d) => d.data().name).filter(Boolean).sort());
    });
    getDocs(collection(db, 'Drs')).then((snap) => {
      setDrNames(snap.docs.map((d) => d.data().name).filter(Boolean).sort());
    });
  }, []);

  useEffect(() => {
    let q = query(collection(db, 'Cases'), orderBy('createdDate', 'desc'), orderBy('createdTime', 'desc'));
    const constraints = [orderBy('createdDate', 'desc'), orderBy('createdTime', 'desc')];
    const wheres = [];
    ...
    return () => unsub();
  }, [selectedClinic, selectedType, selectedDrName, selectedStatus, selectedDate, dueFilter, refreshKey]);
    ...
    <>
      ...
      <PageCard title="View Cases" icon="📋">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
          ...
          <div>
            <label className="block text-sm text-gray-600 mb-1">Date Arrival</label>
            <input type="date" value={selectedDate} onChange={(e) => setSelectedDate(e.target.value)} className="w-full border rounded-md p-2.5 text-black" />
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          {cases.length === 0 && <p className="text-gray-600">No cases found.</p>}
        </div>
      </PageCard>
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="bg-white rounded-xl p-6 max-w-sm w-full">
            <p className="text-black mb-4">Delete this case and its tracking records?</p>
            <div className="flex gap-2 justify-end">
              <button type="button" onClick={() => setDeleteConfirm(null)} className="px-4 py-2 border rounded-md">Cancel</button>
              <button type="button" onClick={() => handleDelete(deleteConfirm)} className="px-4 py-2 bg-red-600 text-white rounded-md">Delete</button>
            </div>
          </div>
        </div>
      )}
...
function CaseCard({ caseData, onManage, onDelete }) {
  const [balance, setBalance] = useState(null);
  const delayed = isDelayed(caseData);

  useEffect(() => {
    if (!caseData.clinicName) return;
    const q = query(collection(db, 'Clinics'), where('name', '==', caseData.clinicName), limit(1));
    const unsub = onSnapshot(q, (snap) => {
  ...
  return (
    <div className={`relative border rounded-xl p-4 bg-white ${delayed ? 'border-red-400' : 'border-gray-200'}`}>
      {delayed && <div className="absolute inset-0 bg-red-500/5 animate-pulse rounded-xl pointer-events-none" />}
      <div className="flex flex-wrap justify-between gap-2 mb-2">
        <div>
          <p className="font-bold text-black">{caseData.clinicName}</p>
          {balance !== null && (
            <p className={`text-sm ${balance >= 0 ? 'text-green-600' : 'text-red-600'}`}>Balance: {formatPriceLE(balance)}</p>
          )}
        </div>
        <div className="flex gap-2">
          <span className="text-xs px-2 py-1 bg-gray-100 rounded text-black">{caseData.caseType}</span>
          <span className="text-xs px-2 py-1 bg-blue-100 rounded text-blue-800">{caseData.status}</span>
        </div>
      </div>
      <div className="grid grid-cols-2 gap-2 text-sm text-black mb-3">
        <p><strong>ID:</strong> {shortId(caseData.id)}</p>
        <p><strong>Price:</strong> {formatPriceLE(caseData.price)}</p>
        <p><strong>Type:</strong> {caseData.type}</p>
        <p><strong>Dr:</strong> {caseData.drName}</p>
        <p><strong>Patient:</strong> {caseData.patientName}</p>
        <p><strong>Due:</strong> {caseData.dueDate || caseData.caseRequestDate}</p>
      </div>
      <div className="flex flex-wrap gap-2">
        <button type="button" onClick={onManage} className="px-3 py-1.5 bg-black text-[#c3a28e] rounded-md text-sm">Manage</button>
        <Link href={`/dashboard/workflow/cases/${caseData.id}`} className="px-3 py-1.5 border rounded-md text-sm text-black">View</Link>
        <button type="button" onClick={onDelete} className="px-3 py-1.5 border border-red-300 text-red-600 rounded-md text-sm">Delete</button>
      </div>
    </div>
  );
}
