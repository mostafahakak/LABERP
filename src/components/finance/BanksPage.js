  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white rounded-xl p-6 max-w-md w-full">
        <h3 className="font-bold text-black mb-2">Transfer from {bank.name}</h3>
        ...
        <div className="mt-3">
          ...
          <select value={destId} onChange={(e) => setDestId(e.target.value)} className="w-full mt-1 px-3 py-2 border rounded-md text-black">
        ...
        <div className="flex gap-2 justify-end mt-4">
          ...
          <button type="button" onClick={submit} className="px-4 py-2 bg-black text-[#c3a28e] rounded-md">Transfer</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white rounded-xl p-6 max-w-lg w-full max-h-[80vh] overflow-y-auto">
        <h3 className="font-bold text-black mb-4">Transfer History — {bankName}</h3>
        <div className="flex gap-2 mb-4">
          <input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="px-2 py-1 border rounded text-black text-sm" />
          <input type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} className="px-2 py-1 border rounded text-black text-sm" />
        </div>
        {transfers.length === 0 ? <p className="text-gray-500">No transfers found.</p> : (
          <div className="space-y-2">
            {transfers.map((t) => (
              <div key={t.id} className="border rounded-lg p-3 text-sm">
                <p className="font-medium text-black">{t.from} → {t.to}</p>
                <p className="text-gray-600">{t.date} {t.time} — {formatPriceLE(t.amount)}</p>
              </div>
            ))}
          </div>
        )}
  ...
  return (
    <>
      ...
      <PageCard
        title="Banks"
        action={
          <button type="button" onClick={() => setShowAdd(true)} className="px-4 py-2 bg-black text-[#c3a28e] rounded-md text-sm">+ Add Bank</button>
        }
      >
              ...
              {list.map((bank) => (
                <div key={bank.id} className="flex flex-wrap items-center justify-between gap-3 border rounded-xl p-4 bg-white">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <button type="button" onClick={() => removeBank(bank.id, bank.name)} className="px-3 py-1.5 border border-red-300 text-red-600 rounded-md text-sm">Delete</button>
                  </div>
                </div>
              ))}
            </div>
        ...
        {banks.length === 0 && <p className="text-gray-500">No banks found.</p>}
      </PageCard>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="bg-white rounded-xl p-6 max-w-md w-full">
            ...
            <div className="flex gap-2 justify-end mt-4">
              ...
              <button type="button" onClick={addBank} className="px-4 py-2 bg-black text-[#c3a28e] rounded-md">Submit</button>
            </div>
          </div>
        </div>
      )}
