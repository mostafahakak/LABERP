          ...
          <div className="space-y-3">
            {utilities.map((u) => (
              <div key={u.id} className="flex items-center justify-between border rounded-xl p-4 bg-white">
                ...
                <button type="button" onClick={() => remove(u.id, u.name)} className="text-destructive px-3 py-1 border border-destructive/30 rounded-md text-sm">Delete</button>
              </div>
            ))}
          </div>
        )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="bg-card rounded-xl p-6 max-w-md w-full">
            ...
            <div className="flex gap-2 justify-end mt-4">
              ...
              <button type="button" onClick={addUtility} className="px-4 py-2 bg-primary text-primary-foreground rounded-md">Submit</button>
            </div>
          </div>
        </div>
      )}
