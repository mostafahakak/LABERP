...
export default function PaySalaries() {
  ...
  useEffect(() => {
    ...
    getDocs(collection(db, "Salaries")).then((snap) => {
      setAllSalaries(snap.docs.map((d) => ({ id: d.id, ...d.data() })));
    });
  }, []);
    ...
    if (paymentType === "Salary") {
      getDocs(
        query(
          ...
          where("userId", "==", selectedRecipientId),
        ),
      ).then((snap) => {
  ...
  const submit = async () => {
    if (!selectedRecipientId || !selectedBankId || paidVal <= 0) {
      setSnack({
        message: "Select recipient, bank, and valid amount",
        isError: true,
      });
      return;
    }
    ...
    try {
      ...
      if (selectedPaymentTypeIsUtility()) {
        ...
        financeData.drPaymentType = "";
        financeData.drPercent = 0;
        await updateDoc(doc(db, "Users", selectedRecipientId), {
      ...
      if (paymentType === "Salary" && selectedSalaryDocId) {
        ...
        if (salDoc) {
          ...
          if (newAmt > 0)
            await updateDoc(doc(db, "Salaries", selectedSalaryDocId), {
              amount: newAmt,
            });
      ...
      if (
        (paymentType === "Salary" || paymentType === "Invoice Payment") &&
      ...
      if (paymentType === "Invoice Payment" && selectedInvoiceId) {
        ...
        if (selectedNotificationUID)
          financeData.selectedNotificationUID = selectedNotificationUID;
      } else {
      ...
      setSnack({
        message: "Payment submitted and balances updated",
        isError: false,
      });
      setPaidAmount("");
      setSelectedRecipientId("");
    ...
    <>
      ...
      <PageCard title="Expense Payment">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          />
          {paymentType === "Salary" && allSalaries.length > 0 && (
            <SelectField
              label="All Salaries"
              value={selectedSalaryDocId}
              onChange={(val) => {
                setSelectedSalaryDocId(val);
                const sal = allSalaries.find((s) => s.id === val);
                if (sal) {
                  setSelectedRecipientId(sal.userId || "");
                  setPaidAmount(String(sal.amount || ""));
                }
              }}
              options={allSalaries.map((s) => s.id)}
          ...
          {paymentType === "Salary" && salaryDocs.length > 0 && (
            <SelectField
              ...
              options={salaryDocs.map(
                (s) => `${s.id.substring(0, 8)} — ${formatPriceLE(s.amount)}`,
              )}
