      ...
      <PageCard title="Patient Invoice">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input list="clients-list" value={name} onChange={(e) => { setName(e.target.value); selectClient(e.target.value); }} className="w-full mt-1 px-3 py-2 border rounded-md text-black" />
      ...
      <PageCard title="Invoice Items">
        <div className="flex flex-wrap gap-2 mb-4">
          {invoiceItems.map((item) => (
            <button key={item.id} type="button" onClick={() => addItem(item)} className="px-3 py-1.5 bg-gray-100 rounded-lg text-sm text-black hover:bg-[#c3a28e]/20">
        ...
        <button type="button" onClick={submit} disabled={loading} className="mt-4 px-6 py-2.5 bg-black text-[#c3a28e] rounded-md font-medium">
          Submit Invoice
        </button>
      </PageCard>
